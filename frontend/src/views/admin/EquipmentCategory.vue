<template>
  <div class="equipment-category-page">
    <!-- 页面标题区域 -->
...
const fetchCategories = async () => {
  try {
    ...
    if (isSearchMode.value && searchKeyword.value.trim()) {
      result = await equipmentCategoryAPI.searchPage(searchKeyword.value.trim(), params);
    } else {
      result = await equipmentCategoryAPI.listPage(params);
    }
...
const handleDelete = async (row) => {
  try {
    ...
    const result = await equipmentCategoryAPI.delete(row.id);
...
const handleSubmit = async () => {
  try {
    ...
    const result = isEdit.value
      ? await equipmentCategoryAPI.update(formData.id, formData)
      : await equipmentCategoryAPI.create(formData);
